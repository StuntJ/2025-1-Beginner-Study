...
- 답: staging area, local repository에 있는 상태
...
- 답: create a merge commit -> 두 브랜치를 부모로 하는 커밋 생성
squash and merge -> 추가할 브랜치의 커밋을 병합하여 원래 브랜치에 추가 
rebase and merge -> 추가할 브랜치의 커밋을 원래 브랜치에 그대로 추가
...
- 답: 가장 최근 커밋은 a1s2d3f이고 여기서 작업이 이루어지는 중이며, 최종 작업은 readme라는 파일을 추가하는 작업이라고 추측할 수 있다.
...
- 답: revert 확실히 잘못됬다고 생각해서 reset으로 지웠는데 잘못된게 아니였다라고 한다면 revert는 수습이 되지만, reset은 수습이 안된다.
